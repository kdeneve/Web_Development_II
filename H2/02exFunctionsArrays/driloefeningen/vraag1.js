...
function left(woord, aantal = 1) {
    console.log(woord.slice(0, aantal));
};
...
console.log(dieren[0]);
console.log(dieren[dieren.length -1]);
...
let woord= '';
for (let teller = 0; teller < words.length; teller++) {
  if (teller === 0)
   woord += words[teller];
  else
   woord += "-" + words[teller];
